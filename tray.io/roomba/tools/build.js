...
const assets = async () => {
  const files = await fs.getFiles('base');
  for (let file of files) {
    if (file.endsWith('.svg') || file.endsWith('.ico')) {
      await fs.copyFile('base/' + file, 'build/' + file);
    }
  }
};
...
const css = async () => {
  const source = await fs.readFile('./base/css/main.css');
...
const html = async () => {
  ...
  for (let file of files) {
    if (file.endsWith('.md')) {
      source = await fs.readFile('base/' + file);
